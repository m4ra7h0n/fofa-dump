...
class FofaSearch:
    ...
    def __init__(self, query_str):
        ...
        self.query = '(' + query_str + ') && country="CN" && status_code="200"'

    def get_all_links(self):
        print(self.query)
        return self.get_all_links_recursion(time_format_to_timestamp(increase(now_timestamp(), 1)))

    def get_all_links_recursion(self, timestamp_before):
        total = self.get_total_size()
        print('fofa查询获取总数量: ' + str(total))
        links = set()
        ...
        while True:
            ...
            if result is not None:
                for link in result['results']:
                    links.add(link)
                last_size = int(result['size'])

                if last_size == 0:
                    break
                print('fofa查询获取link的数量 before->{before}: {num}'.format(before=timestamp_to_time_format(timestamp_before), num=str(len(result['results']))))
                ...
                if len(links) >= total:
                    break
        print('去重后已获取: ' + str(len(links)))
        return links

    def my_fofa(self, query=None, size=10000):
        query = self.query if query is None else query
        qbase64 = base64.b64encode(query.encode('utf-8'))
        url = 'https://fofa.info/api/v1/search/all?email={email}&key={key}&size={size}&fields=link&qbase64={qbase64}&full=true'.format(email=self.email, key=self.key, size=size, qbase64=qbase64.decode('utf'))
